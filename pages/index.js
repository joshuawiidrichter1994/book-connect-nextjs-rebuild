...
export default function Home(props) {
    ...
    list: {
      updateRemaining: () => {
        ...
        DOM.list.button().innerHTML = /* html */ `
               <span>Show more</span>
               <span className=`+styles.list__remaining+`> (${remaining})</span>
           `;
      },
  ...
  const createPreview = (props) => {
    ...
    element.innerHTML = /* html */ `
      <img
          class=`+styles.preview__image+`
          src=`+image+`
      />
      
      <div className=`+styles.preview__info +`>
          <h3 className=`+ styles.preview__title + `>${title}</h3>
          <div className=` + styles.preview__author + `>${authors[authorId]}</div>
      </div>
  `;
  ...
  return (
    <>
      ...
      <dialog className={styles.overlay} data-settings-overlay>
        <div className={styles.overlay__content}>
          <form className={styles.overlay__form} data-settings-form id="settings">
            <label className={styles.overlay__field}>
